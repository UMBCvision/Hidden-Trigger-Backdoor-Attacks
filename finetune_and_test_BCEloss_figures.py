"""
This script finetunes Inception_v3 on clean and poisoned data and evaluates their performance.

ani0075saha @ April 30 2019
"""
...
checkpointDir = "finetuned_models/" + experimentID + "/rand_loc_" +  str(rand_loc) + "/eps_" + str(eps) + \
				"/patch_size_" + str(patch_size) + "/trigger_" + str(trigger_id) + "/BCELoss"
				...
				for inputs, labels in tqdm(dataloaders[phase]):
					...
					with torch.set_grad_enabled(phase == 'train'):
						...
						if is_inception and phase == 'train':
							...
							# Sigmoid
							probs = sigmoid_layer(outputs)
							loss = criterion(outputs, labels)

						# _, preds = torch.max(outputs, 1)
						preds = where(probs >= 0.5, 1, 0)
...
# save final layer weights as mat file
mat_dict = {'fc8_weights': model_ft.classifier[6].weight.detach().cpu().numpy(), 'fc8_bias': model_ft.classifier[6].bias.detach().cpu().numpy()}
scipy.io.savemat(os.path.join(checkpointDir, '{}_fc8_weights_poisoned.mat'.format(experimentID)), mat_dict)
...
# save final layer weights as mat file
mat_dict = {'fc8_weights': model_ft.classifier[6].weight.detach().cpu().numpy(), 'fc8_bias': model_ft.classifier[6].bias.detach().cpu().numpy()}
scipy.io.savemat(os.path.join(checkpointDir, '{}_fc8_weights_clean.mat'.format(experimentID)), mat_dict)
